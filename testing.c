...
int mapX, mapY;      // map width and height
int *map = NULL;     // dynamically allocated map array
...
// Function to fix angle within the range 0-359
int FixAng(float a)
{
    if (a > 359) a -= 360;
    if (a < 0) a += 360;
...
// Function to calculate distance with fisheye correction
float distance(float ax, float ay, float bx, float by, int ang)
{
    return cos(degToRad(ang)) * (bx - ax) - sin(degToRad(ang)) * (by - ay);
}

// Function to draw rays and walls
void drawRays2D()
{
    glColor3f(0, 1, 1);
    glBegin(GL_QUADS);
...
void parseMapFile(const char *filename)
{
    ...
    map = (int *)malloc(mapX * mapY * sizeof(int));
    if (map == NULL)
    {
...
// Function to free allocated map memory
void freeMap()
{
    if (map != NULL)
    {
